...
async def set_repricing_paused(
    redis_service: RedisService, seller_id: str, asin: str, paused: bool
) -> bool:
...
async def is_repricing_paused(
    redis_service: RedisService, seller_id: str, asin: str
) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Error checking repricing_paused flag for {seller_id}:{asin}: {e}"
        )
        return False


async def reset_seller_products(
    redis_service: RedisService, seller_id: str, reason: str = "manual_reset"
) -> Dict[str, Any]:
                ...
                try:
                    ...
                    if reset_success:
                        ...
                        results["errors"].append(
                            f"Failed to reset {asin}:{seller_id}:{sku}"
                        )

                    # Set repricing paused flag
                    pause_success = await set_repricing_paused(
                        redis_service, seller_id, asin, True
                    )
                    ...
                    if pause_success:
                        ...
                        results["error_count"] += 1
                        results["errors"].append(
                            f"Failed to pause repricing for {asin}:{seller_id}"
                        )
                    ...
                    logger.error(
                        f"Error processing product {asin}:{field} for seller {seller_id}: {e}"
                    )
                    results["error_count"] += 1
                    results["errors"].append(
                        f"Error processing {asin}:{field}: {str(e)}"
                    )
    ...
    logger.info(
        f"Reset completed for seller {seller_id}: {results['reset_count']} resets, {results['pause_count']} pauses, {results['error_count']} errors"
    )

    return results


async def resume_seller_products(
    redis_service: RedisService, seller_id: str
) -> Dict[str, Any]:
    """Resume repricing for all products of a specific seller by removing pause flags and setting calculated prices."""
    results = {
        "resume_count": 0,
        "price_set_count": 0,
        "error_count": 0,
    ...
    try:
        ...
        for pause_key in pause_keys:
            try:
                ...
                pause_success = await set_repricing_paused(
                    redis_service, seller_id, asin, False
                )

                price_set_success = False

                if pause_success:
                    results["resume_count"] += 1
                    
                    # Get product data to find all SKUs for this ASIN and seller
                    product_skus = await _get_product_skus_for_asin_seller(
                        redis_service, asin, seller_id
                    )
                    
                    # Set calculated price using max_price for each SKU
                    for sku in product_skus:
                        price_success = await _set_max_price_as_calculated(
                            redis_service, asin, seller_id, sku
                        )
                        if price_success:
                            price_set_success = True
                            results["price_set_count"] += 1

                else:
                    results["error_count"] += 1
                    results["errors"].append(
                        f"Failed to resume repricing for {asin}:{seller_id}"
                    )
                ...
                results["processed_products"].append(
                    {
                        "asin": asin, 
                        "seller_id": seller_id, 
                        "resume_success": pause_success,
                        "price_set_success": price_set_success
                    }
                )
    ...
    logger.info(
        f"Resume completed for seller {seller_id}: {results['resume_count']} resumes, "
        f"{results['price_set_count']} prices set, {results['error_count']} errors"
    )

    return results


async def clear_calculated_price(
    redis_service: RedisService, asin: str, seller_id: str, sku: str
) -> bool:
    ...
    try:
        ...
        if result:
            ...
            logger.info(
                f"No calculated price found to clear for {asin}:{seller_id}:{sku}"
            )
            return True  # Return True since the desired state is achieved

    except Exception as e:
        logger.error(
            f"Error clearing calculated price for {asin}:{seller_id}:{sku}: {e}"
        )
        return False


async def get_seller_reset_rules(
    redis_service: RedisService, seller_id: str
...
async def _get_product_skus_for_asin_seller(
    redis_service: RedisService, asin: str, seller_id: str
) -> list[str]:
    """Get all SKUs for a specific ASIN and seller combination."""
    try:
        redis_client = await redis_service.get_connection()
        skus = []

        # Check current data structure (ASIN_* keys)
        ...
        logger.error(f"Error getting SKUs for {asin}:{seller_id}: {e}")
        return []


async def _set_max_price_as_calculated(
    redis_service: RedisService, asin: str, seller_id: str, sku: str
) -> bool:
    """Set the max_price as the calculated price for a product during resume."""
    try:
        # Get product data to extract max_price
        product_data = await redis_service.get_product_data(asin, seller_id, sku)
        if not product_data:
            logger.warning(f"Product data not found for {asin}:{seller_id}:{sku}")
            return False
        ...
        if max_price is None:
            logger.warning(f"No max_price found for {asin}:{seller_id}:{sku}")
            return False

        # Create calculated price data using max_price as the new price
        calculated_price_data = {
            ...
            "strategy_used": "RESUME_MAX_PRICE",
        ...
        success = await redis_service.save_calculated_price(
            asin, seller_id, sku, calculated_price_data
        )

        if success:
            logger.info(
                f"Set max_price as calculated price for {asin}:{seller_id}:{sku}: "
                f"{current_price} -> {max_price}"
            )
        else:
            logger.error(f"Failed to save calculated price for {asin}:{seller_id}:{sku}")

        return success

    except Exception as e:
        logger.error(f"Error setting max_price as calculated for {asin}:{seller_id}:{sku}: {e}")
        return False

...
class OfferChangeTrigger(BaseModel):
    ...
    offer_change_type: str = Field(
        default="External", description="Type of offer change"
    )
...
class LowestPrice(BaseModel):
    ...
    fulfillment_channel: str = Field(
        ..., description="Fulfillment type (Amazon/Merchant)"
    )
    ...
    landed_price: Optional[float] = Field(
        None, description="Total price including shipping"
    )
...
class Summary(BaseModel):
    ...
    number_of_offers: Optional[List[NumberOfOffers]] = Field(
        default=None, description="Offer counts"
    )
    lowest_prices: Optional[List[LowestPrice]] = Field(
        default=None, description="Lowest prices by condition"
    )
    buy_box_prices: Optional[List[BuyBoxPrice]] = Field(
        default=None, description="Buy box prices"
    )
...
class Offer(BaseModel):
    ...
    is_buy_box_winner: Optional[bool] = Field(
        default=False, description="Is this offer the buy box winner"
    )
    is_featured_merchant: Optional[bool] = Field(
        default=False, description="Is featured merchant"
    )
...
class AmazonOfferChange(BaseModel):
    ...
    # Core identification
    offer_change_trigger: OfferChangeTrigger = Field(
        ..., description="Offer change trigger data"
    )
    ...
    offers: Optional[List[Offer]] = Field(
        default=None, description="List of competing offers"
    )
...
class WalmartOfferChange(BaseModel):
    ...
    current_buybox_price: Optional[float] = Field(
        None, description="Current buybox price"
    )
    current_buybox_winner: Optional[str] = Field(
        None, description="Current buybox winner"
    )
...
class CompetitorInfo(BaseModel):
    ...
    is_buybox_winner: Optional[bool] = Field(
        None, description="Whether this competitor has buybox"
    )
...
class ComprehensiveCompetitionData(BaseModel):
    ...
    lowest_price_competitor: Optional[CompetitorInfo] = Field(
        None, description="Overall lowest price competitor"
    )

    # For LOWEST_FBA_PRICE strategy
    lowest_fba_competitor: Optional[CompetitorInfo] = Field(
        None, description="Lowest FBA competitor"
    )

    # For MATCH_BUYBOX strategy
    buybox_winner: Optional[CompetitorInfo] = Field(
        None, description="Current buybox winner"
    )

    # Additional metadata
    total_offers: Optional[int] = Field(None, description="Total number of offers")
    all_competitors: List[CompetitorInfo] = Field(
        default_factory=list, description="All competitors"
    )
...
class ProcessedOfferData(BaseModel):
    ...
    processed_time: datetime = Field(
        default_factory=lambda: datetime.now(UTC), description="When we processed it"
    )
    ...
    competition_data: ComprehensiveCompetitionData = Field(
        default_factory=ComprehensiveCompetitionData,
        description="Strategy-aware competition data",
    )

    # Primary competitor price field for repricing engine
    competitor_price: Optional[float] = Field(
        None, description="Primary competitor price for repricing decisions"
    )

    # Legacy fields for backward compatibility (deprecated)
    lowest_price: Optional[float] = Field(
        None, description="Lowest competitor price (deprecated)"
    )
    lowest_price_competitor: Optional[str] = Field(
        None, description="Lowest price competitor (deprecated)"
    )
    buybox_winner: Optional[str] = Field(
        None, description="Current buybox winner (deprecated)"
    )
    total_offers: Optional[int] = Field(
        None, description="Total number of offers (deprecated)"
    )

    # Raw data for detailed analysis if needed
    raw_offers: Optional[List[Dict[str, Any]]] = Field(
        None, description="Raw offer data"
    )
...
class RepricingDecision(BaseModel):
    ...
    # Competition data
    competitor_data: ProcessedOfferData = Field(
        ..., description="Processed competition data"
    )

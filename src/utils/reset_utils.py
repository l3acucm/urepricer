...
async def should_skip_repricing_async(
    ...
    try:
        ...
        if skip:
            logger.info(
                f"Skipping repricing for seller {seller_id} - in reset window ({reset_hour:02d}:00-{resume_hour:02d}:00)"
            )
...
def should_skip_repricing_sync(
    seller_id: str, current_time: Optional[datetime] = None
) -> bool:
    """
    Synchronous wrapper for reset rule checking using cached rules.
    ...
    try:
        ...
        # Use the cached sync function from tasks.price_reset
        from tasks.price_reset import should_skip_repricing
        return should_skip_repricing(user_id, market, current_time)

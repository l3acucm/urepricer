...
class MessageProcessor:
    ...
    async def process_amazon_sqs_message(
        ...
        try:
            ...
            self.logger.debug("raw_amazon_sqs_message", raw_message=raw_message)
            message_body = json.loads(raw_message.get("Body", "{}"))
            self.logger.debug("parsed_message_body", message_body=message_body)
            ...
            self.logger.debug("notification_data", notification_data=notification_data)
            ...
            seller_id = trigger_data.get("SellerId") or self._extract_target_seller(
                offers_data, asin
            )
            ...
            processed_data = ProcessedOfferData(
                ...
                competitor_price=competition_data.lowest_price_competitor.price
                if competition_data.lowest_price_competitor
                else None,
                # Legacy fields for backward compatibility
                lowest_price=competition_data.lowest_price_competitor.price
                if competition_data.lowest_price_competitor
                else None,
                lowest_price_competitor=competition_data.lowest_price_competitor.seller_id
                if competition_data.lowest_price_competitor
                else None,
                buybox_winner=competition_data.buybox_winner.seller_id
            ...
            self.logger.error("failed_to_process_amazon_sqs_message", error=str(e))
    ...
    async def process_walmart_webhook(
        ...
        try:
            ...
            _webhook_message = WalmartWebhookMessage(
            ...
            self.logger.error("failed_to_process_walmart_webhook", error=str(e))
    ...
    def _parse_timestamp(self, timestamp_str: Optional[str]) -> datetime:
        ...
        try:
            ...
            self.logger.warning("failed_to_parse_timestamp", timestamp_str=timestamp_str)
            return datetime.now(timezone.utc)
    ...
    def _extract_walmart_competition_data(
        self, offer_change: WalmartOfferChange
    ) -> ComprehensiveCompetitionData:
            ...
            for offer in offer_change.offers:
                ...
                if seller_id != offer_change.seller_id and price:
                    competitor = CompetitorInfo(
                        ...
                        is_buybox_winner=seller_id
                        == offer_change.current_buybox_winner,
            ...
            if offer_change.current_buybox_winner:
                buybox_winner = next(
                    (
                        ...
                        if comp.seller_id == offer_change.current_buybox_winner
                    ),
                    None,
                )
        ...
        if not lowest_price_competitor and offer_change.current_buybox_price:
            if (
                offer_change.current_buybox_winner
                and offer_change.current_buybox_winner != offer_change.seller_id
            ):
                competitor = CompetitorInfo(
    ...
    def _extract_comprehensive_competition_data(
        ...
        lowest_price_competitor = self._find_lowest_price_competitor(
            summary_data, item_condition
        )
        lowest_fba_competitor = self._find_lowest_fba_competitor(
            offers_data, item_condition
        )
    ...
    def _extract_all_competitors(
        self, offers_data: List[Dict[str, Any]], item_condition: str
    ) -> List[CompetitorInfo]:
        """Extract all competitors from offers data."""
        competitors = []
    ...
    def _find_lowest_price_competitor(
        self, summary_data: Dict[str, Any], item_condition: str
    ) -> Optional[CompetitorInfo]:
    ...
    def _find_lowest_fba_competitor(
        self, offers_data: List[Dict[str, Any]], item_condition: str
    ) -> Optional[CompetitorInfo]:
        ...
        for offer in offers_data:
            ...
            offer_condition = offer.get("SubCondition", "").lower()
            if offer_condition != item_condition.lower() or not offer.get(
    ...
    def _extract_price_from_price_info(
    ...
    def _extract_total_offers(self, summary_data: Dict[str, Any]) -> Optional[int]:
        ...
        if number_of_offers:
            return sum(
                offer_count.get("OfferCount", 0) for offer_count in number_of_offers
            )
        return None

    def _extract_target_seller(
        self, offers_data: List[Dict[str, Any]], asin: str
    ) -> str:
        ...
        try:
            ...
            self.logger.warning("could_not_lookup_seller", asin=asin, error=str(e))

...
class RepricingOrchestrator:
    ...
    def __init__(
        ...
        self.repricing_engine = RepricingEngine(redis_service)
        self.logger = logger.bind(service="repricing_orchestrator")
    ...
    async def process_amazon_message(self, raw_sqs_message: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Amazon message processed in {processing_time:.2f}ms",
                extra={
            ...
            self.logger.error(
                f"Failed to process Amazon message: {str(e)}",
                extra={"processing_time_ms": processing_time}
            )
            
            return error_result
    
    async def process_walmart_webhook(self, raw_webhook_payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            # Step 1: Extract and validate message fields
            processed_data = await self.message_processor.process_walmart_webhook(raw_webhook_payload)
            ...
            self.logger.info(
                f"Walmart webhook processed in {processing_time:.2f}ms",
                extra={
            ...
            self.logger.error(
                f"Failed to process Walmart webhook: {str(e)}",
                extra={"processing_time_ms": processing_time}
            )
    ...
    async def process_message_batch(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                self.logger.error(f"Message {i} failed: {str(result)}")
            else:
        ...
        self.logger.info(
            f"Batch processed: {successful_count}/{len(messages)} successful in {batch_time:.2f}ms",
            extra={
    ...
    async def _run_repricing_pipeline(self, processed_data: ProcessedOfferData) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            decision = await self.repricing_engine.make_repricing_decision(processed_data)
            ...
            price_saved = await self.repricing_engine.save_price_if_changed(calculated_price)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self.stats["messages_processed"] > 0:
            avg_time = self.stats["total_processing_time"] / self.stats["messages_processed"]
    ...
    async def shutdown(self):
        ...
        self.logger.info("Shutting down repricing orchestrator...")
